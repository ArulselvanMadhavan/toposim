...
let ring_send_receive n payload all_link_types _all_dst_mats all_link_mats =
  ...
  let term_procs uls dls =
    ...
    in
    let dl_procs =
      Array.map dls ~f:(fun dl -> Process.create [ Signal.id dl ] (dl_proc dl))
    in
  ...
  let switch_procs t_to_sw_lmat sw_to_t_lmat sw_to_sw_lmat =
    ...
    in
    let dls_procs =
      Array.map dls ~f:(fun dl -> Process.create [ Signal.id dl ] (dls_proc dl))
    in
    (* Handle uls *)
    ...
    in
    let uls_procs =
      Array.map uls ~f:(fun ul -> Process.create [ Signal.id ul ] (uls_proc ul))
    in
    Array.append uls_procs dls_procs
  in
